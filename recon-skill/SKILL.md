---
name: recon
description: Pre-build competitive reconnaissance. On build-intent, OFFER it as a one-line question first — do NOT auto-run. You sometimes intentionally build a free version of a known paid product and don't need the whole landscape. When run, it sweeps GitHub + web for existing free and paid options, ranks the top ~10 competitors and best repos in a comparison table (maturity, license, price, install, key features, activity), then finds where an edge exists (or honestly says it's a red ocean). Fires the OFFER whenever the user says "build / create / make / let's do / I want to build / start a new <thing>", or asks "does this exist / what's out there for <thing>". Output is a discussion, not code — building starts only after the verdict. Invoke directly as /recon <thing> to skip the offer and run.
...
- **"No, I know it exists, I want a free version"** (a common, legit move) → skip the landscape sweep, but offer the lighter cut: *"Want the top incumbent's feature list as a build target so the clone hits parity?"* — then build.
- **"Just build it"** → respect it, build, no recon.

Direct `/recon <thing>` skips the offer and runs immediately.

**Never auto-run the full sweep without asking.** Users sometimes build a known duplicate on purpose (a free version of something good). The offer is the safeguard; the call is theirs. Skip the offer entirely for trivial scripts, vault edits, or bespoke glue.
...
### 4. Rank + table
Order by relevance-to-what-the-user-wants (not just popularity). Render a comparison table. **Keep cells narrow — assume an 80-col terminal:** 2-col ≤30 chars, 3-col ≤20, 4+ col → switch to per-competitor bullets. Lead with a one-line "best overall" pick + why.
...
- Do you have an **unfair advantage** here (existing audience, adjacent tool, distribution)?
...
Then **stop and have the conversation.** Don't roll into building. The user decides after seeing the landscape.
...
THE EDGE
<unclaimed niche / gaps / your advantage — or "red ocean, here's why">
...
- Narrow table cells (80-col terminal). Your timezone convention. Your name/spelling rules.
- Output is a discussion + verdict. Building is a separate, later step the user green-lights.
